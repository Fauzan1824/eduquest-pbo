...
export default async function LeaderboardPage() {
  ...
  // Urutan render podium: kiri=#2 (index 1), tengah=#1 (index 0), kanan=#3 (index 2)
  const podiumOrder = [1, 0, 2];
  const podiumHeights = ['h-28', 'h-36', 'h-24']; // kiri, tengah, kanan
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      {leaderboard && leaderboard.length >= 3 && (
        <div className="flex items-end justify-center gap-4 mb-8">
          {podiumOrder.map((dataIndex, renderPos) => {
            const p = leaderboard[dataIndex];
            if (!p) return null;
            const rank = dataIndex + 1; // rank asli: 1, 2, atau 3
            return (
              <div key={p.id} className="flex flex-col items-center gap-2">
                ...
                <div className="text-xs text-center">
                  <div className={clsx('font-bold', rankColors[dataIndex])}>#{rank}</div>
                ...
                {/* Podium block — tinggi berbeda: tengah paling tinggi */}
                <div
                  className={clsx(
                    'w-20 rounded-t-xl flex items-center justify-center',
                    podiumHeights[renderPos],
                    rankBg[dataIndex]
                  )}
                >
                  <Medal className={clsx('w-6 h-6', rankColors[dataIndex])} />
                </div>
              </div>
            );
          })}
            ...
            >
              <div
                className={clsx(
                  ...
                  i < 3 ? rankColors[i] : 'text-slate-500'
                )}
              >
                #{i + 1}
              </div>
